...
def clear_for_summary_with_same_input():
    text2.delete(1.0, END)
    E2.delete(0, 'end')
    E3.delete(0, 'end')
...
def summary():
    ...
    length = len(x)
    clear_for_summary_with_same_input()
...
def summary_gui(txt,n):
    ...
    summarizer gui
    """
    img = ImageTk.PhotoImage(Image.open("textool.jpg"))
    panel = Label(root, image = img)

    h1 = Button(panel,text="Home", command = home,height=2, width=20)
    h1.configure(bg= "#abddee")
    h1.pack()
    h1.place(bordermode=INSIDE,x=30, y=0)

    c1 = Button(panel,text="Close", command = close_window,height=2, width=20)
    c1.configure(bg= "#abddee")
    c1.pack()
    c1.place(bordermode=INSIDE,x=1300, y=30)
...
def main():
   ...
   # panel = Label(root, image = imag)
    ...
    R1.pack( anchor = W )
    R1.place(bordermode=INSIDE,x=520, y=250)
    ...
    R2.pack( anchor = W )
    R2.place(bordermode=INSIDE,x=520, y=450)
